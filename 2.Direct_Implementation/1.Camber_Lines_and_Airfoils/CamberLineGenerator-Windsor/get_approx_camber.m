function camber = get_approx_camber(US, LS, LE, TE, type, n)
%% This function estimates the approximate camber line of an airfoil profile.
% It rotates the airfoil to orient the chord along the x-direction and then
% takes the mean x-y values from the two surfaces to estimate the camber. 
% The points can have a linear or cosine spacing (denser near LE/TE).
% 
...
% 
% Example : 
% camber = get_approx_camber(pressure_surf,suction_surf,LE,TE,'cosine',60)
% 
% Use function get_accurate_camber for a better camber estimation.
...
if type=='cosine'
query_points =  (mx - mn)*(0.5*(1-cos(linspace(0, pi, n)))) + mn;
elseif type=='linear'
query_points =  linspace(mn,mx,n);
end
